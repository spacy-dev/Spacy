    ...
    namespace CG
    {
        namespace Termination
        {
            ...
            double StrakosTichyEnergyError::squaredRelativeError() const noexcept
            {
                ...
                return std::accumulate( scaledGamma2.end() - lookAhead_, scaledGamma2.end(), 0. ) /
                       energyNorm2;
            }

            PreemptiveNormalStepTermination::PreemptiveNormalStepTermination( const Vector& dn0,
                                                                              const Real& Gamma,
                                                                              const Real& rho_elbow,
                                                                              const Real& eta_tau )
            ...
            PreemptiveNormalStepTermination::PreemptiveNormalStepTermination(
                const Vector& dn0, const Real& boundNormds )
            ...
            bool
            PreemptiveNormalStepTermination::terminate( const Vector& cg_iterate,
                                                        Real SquaredErrorNormEstimate,
                                                        Real SquaredSolutionNormEstimate ) const
            {
                ...
                else if ( type_ == Type::Normal )
                {
                    ...
                    auto norm2_dn =
                        norm( dn0_ + w ) *
                        norm( dn0_ +
                              w ); // dn = dn0 + \tilde{dn} (this is NOT in the kernel of C!!)
                    ...
                    if ( norm2_dn < 0.1 * GammaSquare * rho_elbow_ * rho_elbow_ )
                    {
                        ...
                        std::cout << "terminating CG because of dn-stopping criterion: Case 1: "
                                     "Small norm"
                                  << std::endl;
                        std::cout << " |dn|^2 " << norm2_dn << " and "
                                  << 0.1 * GammaSquare * rho_elbow_ * rho_elbow_ << std::endl;
                        terminationStatus_ = "1 ";
                    }

                    // assert that stopping criterion has a proper estimate
                    if ( norm2_tilde_dn_ex < std::numeric_limits< double >::max() &&
                         norm2_dn_err < std::numeric_limits< double >::max() &&
                         ( norm2_offset > norm2_tilde_dn_ex ) )
                    {
                                                                    ...
                                                                    // roots of negative numbers
                        {
                            ...
                            auto taubound =
                                min( 1., sqrt( GammaSquare - norm2_dn ) /
                                             sqrt( GammaSquare -
                                                   ( norm2_offset - norm2_tilde_dn_ex ) ) ) -
                                ( sqrt( norm2_dn_err ) /
                            ...
                            if ( eta_tau_ + 1e-15 < taubound )
                            {
                                std::cout << "terminating CG because of dn-stopping criterion: "
                                          ...
                                          << std::endl;
                                std::cout << "   current error estimate "
                                          << sqrt( norm2_dn_err / norm2_tilde_dn_ex ) << std::endl;
                                terminate = true;
            ...
            bool AdaptiveRelativeEnergyError::errorEstimationTerminate() const
            {
                ...
                for ( unsigned i = lookahead_min;
                      i <= lookAhead_old && scaledGamma2.size() > 2 * lookAhead_; i++ )
                {
                    lookAhead_++;
                    tau =
                        std::accumulate( scaledGamma2.end() - lookAhead_, scaledGamma2.end(), 0. ) /
                ...
                if ( firstLookaheadLower == lookAhead_old )
                {
                    ...
                    {
                        tau = std::accumulate( scaledGamma2.end() - lookAhead_, scaledGamma2.end(),
                                               0. ) /
                              std::accumulate( scaledGamma2.end() - 2 * lookAhead_,
                                               scaledGamma2.end() - lookAhead_, 0. );
                        if ( tau > desiredtau )
                        ...
                        if ( firstLookaheadLower < 0.9 * lookAhead_ &&
                             firstLookaheadLower + 5 < lookAhead_ )
                            break;
                        lookAhead_++;
                    }
            ...
            void AdaptiveRelativeEnergyError::update( double alpha, double qAq, double,
                                                      double rPINVr, const Vector& cg_iterate_ )
            {
                scaledGamma2.push_back( alpha * rPINVr );
                energyNorm2 += alpha * rPINVr;
                stepLength2 = std::abs( qAq );
            ...
            void AdaptiveRelativeEnergyError::update( double alpha, double qAq, double qPq,
                                                      double rPINVr )
            {
                scaledGamma2.push_back( alpha * rPINVr );
                energyNorm2 += alpha * rPINVr;
            ...
            double AdaptiveRelativeEnergyError::getStdDev() const noexcept
            {
                ...
                if ( scaledGamma2.size() > ( 2 * lookAhead_ + no_taus ) )
                {
                    ...
                    {
                        tauvec[ i ] = std::accumulate( scaledGamma2.end() - lookAhead_ - i,
                                                       scaledGamma2.end() - i, 0. ) /
            ...
            AdaptiveRelativeEnergyError::AdaptiveRelativeEnergyError(
                PreemptiveNormalStepTermination st )
            {
                st_ = st;
            }
            ...
            bool AdaptiveRelativeEnergyError::operator()() const
            {
                ...
                bool b = st_.terminate( cg_iterate, getSquaredErrorEstimator(),
                                        getSquaredSolutionEstimator() );
                if ( a )
                    std::cout << " terminating due to relative accuracy" << std::endl;
                if ( b )
                    std::cout << " terminating due to alternative step termination criterion"
                              << std::endl;
