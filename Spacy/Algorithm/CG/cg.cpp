...
namespace Spacy
{
  namespace CG
  {
    ...
    Vector Solver::solve( const Vector& x, const Vector& b ) const
    {
      ...
      else
      {
        ...
        if (is<RegularizeViaCallableOperator>(regularization_))
        {
          ...
          regimpl.computeThetaSuggestion();
        }
        return y;
      }
    }
    ...
    Vector Solver::cgLoop( Vector x, Vector r ) const
    {
      ...
      auto q = Qr;
      auto Pq = r; // required only for regularized or hybrid conjugate gradient methods with precond as regularization
      ...
      for ( unsigned step = 1; true; step++ )
      {
        ...
        auto Rq = Pq; // for regularized with callable operator
        if (is<RegularizeViaCallableOperator>(regularization_))
        {
          auto& regimpl = cast_ref<RegularizeViaCallableOperator> (regularization_);
                                          ...
                                          unsigned step ) const
    {
      ...
      {
        auto& regimpl = cast_ref<RegularizeViaCallableOperator> (regularization_);
        auto Rq = regimpl.getRegularization()(q);
        regimpl.update(qAq,Rq(q));
      }
      else
