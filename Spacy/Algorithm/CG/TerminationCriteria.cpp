...
namespace Spacy::CG::Termination
{
    ...
    PreemptiveNormalStepTermination::PreemptiveNormalStepTermination( Vector dn0, Real Gamma, Real rho_elbow, Real eta_tau )
        : dn0_( std::move( dn0 ) ), Gamma_( std::move( Gamma ) ), rho_elbow_( std::move( rho_elbow ) ), eta_tau_( std::move( eta_tau ) )
    {
        type_ = Type::Normal;
    }
        ...
        if ( type_ == Type::Normal )
        {
                 ...
                 ( norm2_offset > norm2_tilde_dn_ex ) )
            {
                                                            ...
                                                            // roots of negative numbers
                {
                    ...
                    auto taubound = min( 1., sqrt( GammaSquare - norm2_dn ) / sqrt( GammaSquare - ( norm2_offset - norm2_tilde_dn_ex ) ) ) -
    ...
    void AdaptiveRelativeEnergyError::update( double alpha, double qAq, double /*unused*/, double rPINVr, const Vector& cg_iterate_ )
    {
        scaledGamma2.push_back( alpha * rPINVr );
        energyNorm2 += alpha * rPINVr;
        stepLength2 = std::abs( qAq );
