...
namespace Spacy
{
    namespace Log
    {
        namespace
        {
            ...
            {
                auto max_size = 15u;
                auto offset = ( name.size() > max_size )
                                  ? std::string()
                                  : std::string( max_size - name.size(), ' ' );
                return name += offset;
            }
        }
        ...
        void StreamPrinter::operator()( const char* tag, const char* name,
        ...
        void FilePrinter::operator()( const char* tag, const char* name,
                                      const Printable& printable )
        {
        ...
        bool Logger::is_disabled( const char* tag ) const
        {
            return std::find( begin( disabled_ ), end( disabled_ ),
                              std::pair< const std::string, bool >( tag, true ) ) !=
                   end( disabled_ );
        }
    }
}
