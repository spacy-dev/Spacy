...
namespace Spacy
{
    namespace Kaskade
    {
        ...
        namespace Lagrange
        {
             ...
             * @brief Creator for conjugate gradient solver for optimal control problems with a
             * CG::TriangularStateConstraintPreconditioner.
             *
             * Solve a system of the form
             * \f[ A = \left( \begin{array}{ccc} L_{yy} & L_{yu}  & A^T \\ L_{uy} & L_{uu} & B^T \\
             * A & B &  \end{array} \right)\f],
             * with preconditioner
             * \f[ P = \left( \begin{array}{ccc}  &  & A^{-T} \\  & L_{uu}^{-1} & -B^T \\ A^{-1} &
             * -B &  \end{array} \right)\f],
             * where \f$A\f$ is the state operator, \f$B\f$ the control operator and \f$L_{uu}\f$ is
             * the second derivative of the Lagrange functional
             * with respect to the control variable \f$u\f$. The state variable is denoted by
             * \f$y\f$ and the adjoint variable by\f$p\f$.
             *
            ...
            public:
                ...
                /**
                 * @brief Create conjugate gradient solver with triangular state constraint
                 * preconditioner for \f$L''\f$.
                 * @param L Lagrange functional
                 * @return solver representing \f$L''(x)^{-1}\f$, where \f$x\f$ is the current
                 * iterate
                 */
