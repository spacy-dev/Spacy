...
namespace Spacy::Kaskade
{
    ...
    class HierarchicalErrorEstimator
    {
        ...
        using ExtensionVariable = ::Kaskade::Variable< ::Kaskade::SpaceIndex< extensionSpaceIndex >, ::Kaskade::Components< Variable::m >,
                                                       ::Kaskade::VariableId< Variable::id > >;
        using ConstSpacePtr =
            std::remove_reference_t< typename boost::fusion::result_of::at_c< typename Descriptions::Spaces, Variable::spaceIndex >::type >;
    ...
    private:
        ...
        double getErrorLevel( std::vector< double > errorDistribution, double maxErr, double norm_x )
        {
            ...
            const auto baseErrLevel = std::max( errorDistribution[ minRefineIndex ], norm_x * std::numeric_limits< double >::epsilon() );
        ...
        void updateErrorIndicator( const CoefficientVectors& estSol, double norm_x )
        {
            ...
            std::transform( gridManager.grid().leafGridView().template begin< 0 >(), gridManager.grid().leafGridView().template end< 0 >(),
