...
namespace Spacy
{
    namespace Kaskade
    {
        ...
        {
        public:
            explicit RefineCells( GridManager& gridManager ) noexcept : gridManager( gridManager )
            {
            }
            ...
            TransferVector operator()( const Spacy::ErrorIndicator& indicator )
            {
                ...
                std::for_each( gridManager.grid().leafGridView().template begin< 0 >(),
                               gridManager.grid().leafGridView().template end< 0 >(), [ this, &cellIndicator ]( const auto& cell ) {
                                   if ( cellIndicator[ gridManager.grid().leafIndexSet().index( cell ) ] )
                                       gridManager.mark( 1, cell );
                               } );
                gridManager.adaptAtOnce();

                return []( const Spacy::Vector& /*unused*/ ) {};
            }
