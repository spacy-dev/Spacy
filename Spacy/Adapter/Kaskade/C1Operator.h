...
namespace Spacy::Kaskade
{
    ...
    public:
         ...
         */
        C1Operator( const OperatorDefinition& f, const VectorSpace& domain, const VectorSpace& range, int rbegin = 0,
                    int rend = OperatorDefinition::AnsatzVars::noOfVariables, int cbegin = 0,
                    int cend = OperatorDefinition::TestVars::noOfVariables )
            : OperatorBase( domain, range ), f_( f ), spaces_( extractSpaces< AnsatzVariableSetDescription >( domain ) ), rbegin_( rbegin ),
              rend_( rend ), cbegin_( cbegin ), cend_( cend ),
        ...
        C1Operator( const C1Operator& B )
            : OperatorBase( B ), NumberOfThreads( B ), f_( B.f_ ), spaces_( B.spaces_ ), A_( B.A_ ), rhs_( B.rhs_ ), old_X_A_( B.old_X_A_ ),
              old_X_dA_( B.old_X_dA_ ), onlyLowerTriangle_( B.onlyLowerTriangle_ ), rbegin_( B.rbegin_ ), rend_( B.rend_ ),
              cbegin_( B.cbegin_ ), cend_( B.cend_ ), solverCreator_( B.solverCreator_ ), operatorSpace_( B.operatorSpace_ )
        {
        }
    ...
    private:
        ...
        void assembleOperator( const ::Spacy::Vector& x ) const
        {
            ...
            rhs_ = zero( range() );
            copy< TestVariableSetDescription >( VectorImpl( assembler_.rhs() ), rhs_ );
     ...
     * @return @ref C1Operator "::Spacy::Kaskade::C1Operator<OperatorDefinition>( A , domain ,
     * range , rbegin , rend , cbegin , cend )"
     *
     * The optional parameters rbegin, rend, cbegin and cend can be used to define operators
     * that correspond to parts of
     * a system of equation.
     */
    template < class OperatorDefinition >
    ::Spacy::C1Operator makeC1Operator( const OperatorDefinition& f, const VectorSpace& domain, const VectorSpace& range, int rbegin = 0,
                                        int rend = OperatorDefinition::AnsatzVars::noOfVariables, int cbegin = 0,
                                        int cend = OperatorDefinition::TestVars::noOfVariables )
    {
     ...
     */
    template < class OperatorDefinition >
    auto makeC1Operator( const OperatorDefinition& f, const VectorSpace& domain, int rbegin = 0,
                         int rend = OperatorDefinition::AnsatzVars::noOfVariables, int cbegin = 0,
                         int cend = OperatorDefinition::TestVars::noOfVariables )
    {
