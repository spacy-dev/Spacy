...
namespace Spacy
{
    namespace Rn
    {
        ...
        CompositeStepFunctional< role >::CompositeStepFunctional(
            ...
            : Spacy::FunctionalBase( domain ), f_( f ), df_( df ), ddf_( ddf ), c_( c ), dc_( dc ),
              pddc_( pddc )
        {
        ...
        CompositeStepFunctional< role >::CompositeStepFunctional(
            ...
            : Spacy::FunctionalBase( L_T.domain() ), f_( L_T.f_ ), df_( L_T.df_ ), c_( L_T.c_ ),
              dc_( L_T.dc_ ), M_( M )
        {
        ...
        {
            ::Eigen::VectorXd x_ =
                cast_ref< Spacy::Rn::Vector >( Spacy::primalComponent( x ) ).get();
        ...
        template < RoleOfFunctional role >
        Spacy::Vector CompositeStepFunctional< role >::d2( const ::Spacy::Vector& x,
        ...
        template < RoleOfFunctional role >
        ::Spacy::Rn::LinearOperator
        CompositeStepFunctional< role >::hessian( const ::Spacy::Vector& x ) const
        {
            ::Eigen::VectorXd x_ =
                cast_ref< Spacy::Rn::Vector >( Spacy::primalComponent( x ) ).get();
            ::Eigen::VectorXd p = cast_ref< Spacy::Rn::Vector >( Spacy::dualComponent( x ) ).get();
