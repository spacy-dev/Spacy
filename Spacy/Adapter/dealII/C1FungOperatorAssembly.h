...
namespace Spacy
{
    namespace dealII
    {
        namespace Detail
        {
            ...
            struct LocalFunGOperatorAssembly
                : LocalAssemblyBase< dim, row, row_dim, column, column_dim >
            {
                ...
                {
                    A.template update< row >( std::make_tuple(
                ...
                void update_gradient_and_hessian( FunGOperator& A, Index q_index )
                {
                    ...
                    {
                        this->cell_rhs( i ) += A() * row_value_view.gradient( i, q_index ) *
                                               this->row_fe_values.JxW( q_index );
                        this->cell_rhs( i ) -= 1 * row_value_view.value( i, q_index ) *
                                               this->row_fe_values.JxW( q_index );
                        ...
                        for ( Index j = 0; j < this->column_dofs_per_cell_; ++j )
                        {
                            this->cell_matrix( i, j ) +=
                                A.template d1< row >(
                                    std::make_tuple( column_value_view.value( j, q_index ),
                                                     column_value_view.gradient( j, q_index ) ) ) *
                                column_value_view.gradient( i, q_index ) *
                                this->row_fe_values.JxW( q_index );
                        }
                    }
                }
            ...
            struct OperatorUpdate
            {
                ...
                std::conditional_t< row == 0, OperatorUpdate< dim, VariableDims,
                                                              VariableDims::size - 1, column - 1 >,
                                    OperatorUpdate< dim, VariableDims, row - 1, column > >
                    next_;
            };
            ...
            {
                explicit OperatorUpdate( const VectorSpace&, const dealii::FESystem< dim >& )
                {
                }
