...
namespace Spacy
{
    namespace dealII
    {
        template < int dim, class VariableDims, int id = VariableDims::size - 1,
                   bool is_product_space = VariableDims::size != 1 >
        ...
        struct InitBlocks
        {
            static void apply( const VectorSpace& V, dealii::BlockDynamicSparsityPattern& dsp )
            {
                dsp.block( row, column )
                    .reinit( creator< VectorCreator< dim, GetDim< row, VariableDims >::value > >(
                                 extractSubSpace( V, row ) )
                                 .degreesOfFreedom(),
                             creator< VectorCreator< dim, GetDim< column, VariableDims >::value > >(
                                 extractSubSpace( V, column ) )
                                 .degreesOfFreedom() );
                InitBlocks< dim, VariableDims, row == 0 ? VariableDims::size - 1 : row - 1,
        ...
        {
            static void apply( const VectorSpace&, dealii::BlockDynamicSparsityPattern& )
            {
            }
        };
        ...
        {
            static void apply( const VectorSpace& V, const dealii::DoFHandler< dim >& dof_handler,
        ...
        {
            static void apply( const VectorSpace&, const dealii::DoFHandler< dim >& dof_handler,
