    ...
    namespace OCP{
      ...
      public:
        ...
        /**
               ...
               */
        ErrorEstimator(::Spacy::Real tolerance, ::Spacy::Real tau):tolerance_(tolerance),tau_(tau) {}


        /**
               * @brief Goal oriented error estimator
               * Estimate I(y_k,u_k)-I(y,u), where (x,u) exact solution (y_k,u_k) time discrete solution
               * I is a functional integrating over [0,\tau] \times \Omega
               * @param x Solution for which error should be estimated
               * @param tau end time for goal oriented error estimation
               * @param c2func C2Functional associated with the optimal control problem (Tangential Step Functional)
               * @return boolean if refimenent is needed to get the indicators below the tolerance
               */
        ...
        bool estimateErr(const ::Spacy::Vector& x,const ::Spacy::C2Functional& c2func)
        {
          ...
          if(Spacy::is<::Spacy::KaskadeParabolic::C2Functional<FunctionalDefinition> >(c2func))
          {
            ...
            {
              errorIndicators.at(i) = errind.at(0).at(i) + errind.at(1).at(i) + errind.at(2).at(i) + errind.at(3).at(i) + errind.at(4).at(i) + errind.at(5).at(i);
              std::cout<<c2func_impl.getGridMan().getTempGrid().getVertexVec().at(i-1)<<" to "<<c2func_impl.getGridMan().getTempGrid().getVertexVec().at(i)<<" "<<errorIndicators.at(i)<<std::endl;
              if(errorIndicators.at(i) > tolerance_) needsRefinement_ = true;
            }

            std::ofstream obj;
            obj.open("data/grid_err"+std::to_string(ctr_) + ".txt");
            for(auto i = 1u;i <no_time_steps ; i++)
              obj << c2func_impl.getGridMan().getTempGrid().getVertexVec().at(i-1) <<" "<<errorIndicators.at(i)<<std::endl;
            obj << c2func_impl.getGridMan().getTempGrid().getVertexVec().at(no_time_steps-1) <<" "<<0<<std::endl;
            obj.close();
        ...
        /**
               ...
               * Estimate J(y_k,u_k)-J(y,u), where (x,u) exact solution (y_k,u_k) time discrete solution
               * @param x Solution for which error should be estimated
               * @param tau end time for goal oriented error estimation
               * @param c2func C2Functional associated with the optimal control problem (Tangential Step Functional)
               * @return boolean if refimenent is needed to get the indicators below the tolerance
               */
        template<class FunctionalDefinition>
        bool estimateErrCostFunc(const ::Spacy::Vector& x,const ::Spacy::C2Functional& c2func)
        {
          needsRefinement_ = false;
          ...
          if(Spacy::is<::Spacy::KaskadeParabolic::C2Functional<FunctionalDefinition> >(c2func))
          {
            ...
            for(auto i = 1u;i<no_time_steps;i++)
            {
              errorIndicators.at(i) = errind.at(0).at(i) + errind.at(1).at(i) + errind.at(2).at(i);
              std::cout<<c2func_impl.getGridMan().getTempGrid().getVertexVec().at(i-1)<<" to "<<c2func_impl.getGridMan().getTempGrid().getVertexVec().at(i)<<" "<<errorIndicators.at(i)<<std::endl;
              if(errorIndicators.at(i) > tolerance_) needsRefinement_ = true;
            }
            std::ofstream obj;
        ...
        void refine(::Spacy::VectorSpace& domain, ::Spacy::C2Functional& c2func, ::Spacy::C2Functional& nfunc)
        {
          ...
          if(Spacy::is<::Spacy::KaskadeParabolic::C2Functional<FunctionalDefinition> >(c2func))
          {
            ...
            auto& c2func_impl = ::Spacy::cast_ref<::Spacy::KaskadeParabolic::C2Functional<FunctionalDefinition> >(c2func);
            auto& nfunc_impl = ::Spacy::cast_ref<::Spacy::KaskadeParabolic::C2Functional<NormFunctionalDefinition> >(nfunc);
          ...
          else std::cout<<"No valid implementation of C2Functional given"<<std::endl;
        }
      private:
