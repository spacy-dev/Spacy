           ...
           */
                      ...
                      spacesVec_( H.getSpacesVec() ), dtVec_( H.getDtVec() )
                {
                    ...
                    for ( auto i = 0u; i < dtVec_.size(); i++ )
                    {
                        ...
                        if ( i != 0 )
                        {
                            //                writeMatlab(H_.getKaskOp("A",
                            //                i).get(),"A"+std::to_string(i));
                            //                writeMatlab(H_.getKaskOp("Mass_yy",
                            //                i).get(),"Mass"+std::to_string(i));
                        ...
                        {
                            //   writeMatlab(H_.getKaskOp("Mu",
                            //   i).get(),"Mubefore"+std::to_string(i));
...
//                         x_p_coeff.at( i ).read( xp.begin() );
                        ...
                        // std::cout<<"Solution of adjoint at time "<<i<<" :
                        // "<<x_p.at(i).two_norm()<<std::endl;
                    }
                    ...
                    for ( auto i = 0u; i < dtVec_.size(); i++ )
                    {
                        ...
                        //            std::cout <<i<< " RHS STATE " << b_p.at(i).two_norm() <<
                        //            std::endl;
                        solDiag.at( i )->apply( b_p_coeff.at( i ), x_y_coeff.at( i ) );
                        //            std::cout <<i<< " SOL STATE " << x_y.at(i).two_norm() <<
                        //            std::endl;
                    ...
                    for ( auto i = 0u; i < dtVec_.size(); i++ )
                    {
                        res_y_impl.getCoeffVec_nonconst( i ) =
                            boost::fusion::at_c< 0 >( x_y_coeff.at( i ).data );
                        res_u_impl.getCoeffVec_nonconst( i ) =
                            boost::fusion::at_c< 0 >( x_u_coeff.at( i ).data );
                        res_p_impl.getCoeffVec_nonconst( i ) =
                            boost::fusion::at_c< 0 >( x_p_coeff.at( i ).data );
                    }
                    return result;
                }
