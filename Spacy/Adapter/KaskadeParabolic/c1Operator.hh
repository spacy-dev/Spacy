...
namespace Spacy
{
    ...
    {
        /**
             * @brief %Operator interface for %Kaskade 7. Models a dynamical differentiable operator
         * \f$A:X\rightarrow Y\f$ semi discretized in time.
             * @see Spacy::C1Operator
             */
        template < class OperatorDefinition >
        class C1Operator : public OperatorBase, public Mixin::NumberOfThreads
        {
            ...
            using TCoeff = typename TestVariableSetDescription::
            ...
            using KaskadeOperator = ::Kaskade::MatrixRepresentedOperator< Matrix, CoefficientVector,
                                                                          CoefficientVector >;
            using Linearization =
                PDE::LinearBlockOperator< VariableSetDescription, VariableSetDescription >;
            ...
            using VYSetDescription =
                ::Spacy::KaskadeParabolic::Detail::ExtractDescription_t< VariableSetDescription,
                                                                         0 >;
        ...
        public:
            ...
            C1Operator( std::function< OperatorDefinition(
                            const typename OperatorDefinition::SourceVars::VariableSet ) >& F,
                        GridManager< typename OperatorDefinition::OriginVars::Spaces >& gm,
                        const VectorSpace& domain, const VectorSpace& range,
                        const Spacy::Vector& u )
                : OperatorBase( domain, range ), F_( F ), gm_( gm ),
                  y0_( VariableSetDescription::template CoefficientVectorRepresentation<>::init(
        ...
        private:
            ...
            {
                if ( old_X_A_ && ( old_X_A_ == x ) && OpRefined_ )
                    return;
                ...
                for ( auto timeStep = 0u; timeStep < dtVec.size(); timeStep++ )
                {
                    ...
                    if ( verbose )
                        std::cout << "assembling elliptic part for timestep " << timeStep
                                  << std::endl;

                    Assembler assembler( *( spacesVec.at( timeStep ) ) );

                    assembler.assemble(::Kaskade::linearization( FVec_.at( timeStep ),
                                                                 x_impl.get_nonconst( timeStep ) ),
                                       Assembler::RHS, getNumberOfThreads() );

                    rhs_impl.getCoeffVec_nonconst( timeStep ) =
                        boost::fusion::at_c< 0 >( CoefficientVectorY( assembler.rhs() ).data );
                    // multiply by dt (Integration of constant function)
                    rhs_impl.get_nonconst( timeStep ) *= get( dtVec.at( timeStep ) );

                    // Time Transfer Terms
                    if ( verbose )
                        std::cout << "assembling transfer part for timestep " << timeStep
                                  << std::endl;
                    ...
                    if ( timeStep == 0 )
                    {
                        ...
                        //            x0(VariableSetDescription(*(spacesVec.at(0)),{"y"}));
                    ...
                    else
                    {
                        ...
                        boost::fusion::at_c< 0 >( y_before_.data ) =
                            x_impl.getCoeffVec( timeStep - 1 );
                    ...
                    rhs_impl.getCoeffVec_nonconst( timeStep ) +=
                        boost::fusion::at_c< 0 >( y_curr_dual.data );
                }
                old_X_A_ = x;
                OpRefined_ = true;
            }
            ...
            {
                if ( old_X_dA_ && ( old_X_dA_ == x ) && GradRefined_ )
                    return;
                ...
                for ( auto timeStep = 0u; timeStep < dtVec.size(); timeStep++ )
                {
                    ...
                    Assembler assembler( *( spacesVec.at( timeStep ) ) );
                    assembler.assemble(
                        ::Kaskade::linearization( FVec_.at( timeStep ), x_impl.get( timeStep ) ),
                        Assembler::MATRIX, getNumberOfThreads() );
                    A_.at( timeStep ) = KaskadeOperator(
            ...
            void assembleMassMatrices() const
            {
                ...
                for ( auto timeStep = 0u; timeStep < dtVec.size(); timeStep++ )
                {
                    ...
                    // assemblersp.assemble(::Kaskade::linearization(this->SP_,x_) ,
                    // Assembler::MATRIX , getNumberOfThreads() );
                    assemblersp.assemble(::Kaskade::linearization( SP_, x_ ), Assembler::MATRIX,
                                         getNumberOfThreads() );

                    MassY_.at( timeStep ) =
                        KaskadeOperator( assemblersp.template get< Matrix >( onlyLowerTriangle_ ) );
            ...
            void makeGradientLBOPointer() const
            {
                ...
                G_ptr = std::make_shared<
                    PDE::LinearBlockOperator< VariableSetDescription, VariableSetDescription > >(
                    PDE::LinearBlockOperator< VariableSetDescription, VariableSetDescription >(
                        MassY_, A_, gm_.getTempGrid().getDtVec(), gm_.getSpacesVec(),
                        VectorSpace( PDE::LinearBlockOperatorCreator< VariableSetDescription,
            ...
            void makeMassLBOPointer() const
            {
                ...
                M_ptr = std::make_shared<
                    PDE::NormOperator< VariableSetDescription, VariableSetDescription > >(
                    PDE::NormOperator< VariableSetDescription, VariableSetDescription >(
                        MassY_, gm_.getTempGrid().getDtVec(), gm_.getSpacesVec(),
                        VectorSpace( PDE::NormOperatorCreator< VariableSetDescription,
                                                               VariableSetDescription >(
                                         this->domain(), this->range() ),
                                     normfunc, true ) ) );
                if ( verbose )
            ...
            std::function< OperatorDefinition(
                const typename OperatorDefinition::SourceVars::VariableSet ) >
                F_;
            ...
            std::function< LinearSolver( const Linearization& ) > solverCreator_ = [](
                const Linearization& M ) { return PDE::DirectSolver< OperatorDefinition >( M ); };
        };

        /**
             * @brief Convenient generation of a differentiable operator \f$A: X\rightarrow Y\f$
         * from %Kaskade 7.
             * @param F operator definition from %Kaskade 7
             * @param gm Grid manager holding temporal and spatial grids
             * @param domain domain space \f$X\f$
             * @param range range space \f$Y\f$
             * @return @ref C1Operator "::Spacy::KaskadeParabolic::C1Operator<OperatorDefinition>(
         * F, gm, domain, range)"
             *
             */
        template < class OperatorDefinition >
        auto makeC1Operator( std::function< OperatorDefinition(
                                 const typename OperatorDefinition::SourceVars::VariableSet ) >& F,
                             GridManager< typename OperatorDefinition::OriginVars::Spaces >& gm,
                             const VectorSpace& domain, const VectorSpace& range,
                             const ::Spacy::Vector& u )
