...
public:
  ...
  public:
    DomainCache(StepFunctional const& f_,
      ...
      f(f_), vars(vars_), c(f.d,f.mu), J(f.J)
    {}
  ...
  private:
    ...
    LinearModelConstraint<AnsatzVars,yIdx> c;
  ...
  explicit StepFunctional(Scalar alpha_, typename AnsatzVars::VariableSet const& ref, Scalar d_=1, Scalar mu_ = 1) :
    gamma(1e9), d(d_), mu(mu_), alpha(alpha_), J(alpha,ref)
  {
    assert(gamma >= 0);
  }
