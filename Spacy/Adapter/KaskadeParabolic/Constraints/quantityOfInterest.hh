...
/**
     ...
     */
template <int stateId, int controlId, int adjointId, class RType, class AnsatzVars_, class TestVars_=AnsatzVars_, class OriginVars_=AnsatzVars_, bool lump=false>
...
public:
  ...
  static ::Kaskade::ProblemType const type = std::is_same<AnsatzVars,TestVars>::value ? ::Kaskade::VariationalFunctional : ::Kaskade::WeakFormulation;
  ...
  class DomainCache : public ::Kaskade::CacheBase<QuantityOfInterest,DomainCache>
  {
  public:
    DomainCache(QuantityOfInterest const& f_,
    ...
    {
      if(row==yIdx) return y*arg.value;
      if(row==uIdx) return u*arg.value;
...
//      if(row==uIdx) return arg.value;
    ...
    Scalar d2_impl (::Kaskade::VariationalArg<Scalar,dim,TestVars::template Components<row>::m> const &arg1, ::Kaskade::VariationalArg<Scalar,dim,TestVars::template Components<col>::m> const &arg2) const
    {
      return 0;
    }

  private:
    QuantityOfInterest const& f;
  ...
  class BoundaryCache : public ::Kaskade::CacheBase<QuantityOfInterest,BoundaryCache>
  {
  public:
    BoundaryCache(QuantityOfInterest const& f,
                  typename AnsatzVars::VariableSet const& vars_, int flags=7):
      vars(vars_)
    {}
    ...
    template<int row, int col>
    Scalar d2_impl (::Kaskade::VariationalArg<Scalar,dim,TestVars::template Components<row>::m> const &arg1, ::Kaskade::VariationalArg<Scalar,dim,AnsatzVars::template Components<col>::m> const &arg2) const
    {
      return 0;
    }
  ...
  {
    static bool const present = !( (row==yIdx && col==yIdx) ||
                                   ( row == pIdx && col == pIdx ) ||
                                   ( row == yIdx && col == uIdx ) ||
                                   ( row == uIdx && col == yIdx ) );
