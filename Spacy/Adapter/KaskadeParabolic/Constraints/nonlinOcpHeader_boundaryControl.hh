...
/**
     * @brief class for stepfunctional definition for optimal control problems solved by composite
 * step
     */
...
class BoundaryControlStepFunctional
{
public:
    ...
    class DomainCache : public CacheBase< BoundaryControlStepFunctional, DomainCache >
    {
    public:
        DomainCache( BoundaryControlStepFunctional const& f_,
    ...
    private:
        double kappa;
        BoundaryControlStepFunctional const& f;
    ...
    class BoundaryCache : public CacheBase< BoundaryControlStepFunctional, BoundaryCache >
    {
    public:
        BoundaryCache( BoundaryControlStepFunctional const& f,
                       typename AnsatzVars::VariableSet const& vars_, int flags = 7 )
            : vars( vars_ ), gamma( f.gamma ), J( f.J )
        {
        }
            ...
            const
        {
            ...
            if ( row == uIdx )
                return J.template d1< uIdx >( arg ) - p * if_( arg.value, u );
            if ( row == pIdx )
                return -u * if_( arg.value, p );
                     ...
                     arg2 ) const
        {
            ...
            if ( row == pIdx && col == uIdx )
                return -if_( arg1.value, p ) * if_( arg2.value, u );
            if ( row == uIdx && col == pIdx )
                return -if_( arg1.value, u ) * if_( arg2.value, p );
    ...
    explicit BoundaryControlStepFunctional( Scalar alpha_,
                                            typename AnsatzVars::VariableSet const& ref,
                                            Scalar c_ = 1, Scalar d_ = 1, Scalar e_ = 0,
                                            Scalar mu_ = 1 )
...
using NormalBoundaryControlStepFunctional =
    BoundaryControlStepFunctional< stateId, controlId, adjointId, RType, AnsatzVars, TestVars,
                                   OriginVars, RoleOfFunctional::NORMAL >;

template < int stateId, int controlId, int adjointId, class RType, class AnsatzVars,
           class TestVars = AnsatzVars, class OriginVars = AnsatzVars >
using TangentialBoundaryControlStepFunctional =
    BoundaryControlStepFunctional< stateId, controlId, adjointId, RType, AnsatzVars, TestVars,
                                   OriginVars, RoleOfFunctional::TANGENTIAL >;
