...
/**
     * @brief class for stepfunctional definition for optimal control problems solved by composite step
     */
...
public:
  ...
  public:
    ...
    Scalar d1_impl (VariationalArg<Scalar,dim, TestVars::template Components<row>::m> const& arg) const
    {
      ...
      if(row==pIdx) return c.template d1<yIdx>(arg);
      return 0;
    }
    ...
    Scalar d2_impl (VariationalArg<Scalar,dim,TestVars::template Components<row>::m> const &arg1, VariationalArg<Scalar,dim,TestVars::template Components<col>::m> const &arg2) const
    {
      ...
      if(row==pIdx && col==uIdx) return 0;
      if(row==uIdx && col==pIdx) return 0;
  ...
  public:
    ...
    Scalar d1_impl (VariationalArg<Scalar,dim,TestVars::template Components<row>::m> const& arg) const
    {
      ...
      if(row==uIdx) return J.template d1<uIdx>(arg) -p*if_(arg.value,u);
      if(row==pIdx) return -u*if_(arg.value,p);
    ...
    Scalar d2_impl (VariationalArg<Scalar,dim,TestVars::template Components<row>::m> const &arg1, VariationalArg<Scalar,dim,AnsatzVars::template Components<col>::m> const &arg2) const
    {
      ...
      if(row==pIdx && col==uIdx) return - if_(arg1.value,p) * if_(arg2.value,u);
      if(row==uIdx && col==pIdx) return - if_(arg1.value,u) * if_(arg2.value,p);
  ...
  explicit BoundaryControlStepFunctional(Scalar alpha_, typename AnsatzVars::VariableSet const& ref,Scalar c_=1, Scalar d_=1, Scalar e_=0, Scalar mu_ = 1) :
...
using NormalBoundaryControlStepFunctional = BoundaryControlStepFunctional<stateId,controlId,adjointId,RType,AnsatzVars,TestVars,OriginVars,RoleOfFunctional::NORMAL>;

template <int stateId, int controlId, int adjointId, class RType, class AnsatzVars, class TestVars=AnsatzVars, class OriginVars=AnsatzVars>
using TangentialBoundaryControlStepFunctional = BoundaryControlStepFunctional<stateId,controlId,adjointId,RType,AnsatzVars,TestVars,OriginVars,RoleOfFunctional::TANGENTIAL>;
