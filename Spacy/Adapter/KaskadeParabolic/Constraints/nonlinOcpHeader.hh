...
/**
     * @brief class for nonlinear stepfunctional definition for optimal control problems solved by
 * composite step
     */
template < int stateId, int controlId, int adjointId, class RType, class AnsatzVars_,
           ...
           RoleOfFunctional role = RoleOfFunctional::NORMAL, bool lump = false >
...
public:
    ...
    static ProblemType const type =
        std::is_same< AnsatzVars, TestVars >::value ? VariationalFunctional : WeakFormulation;
    ...
    static int const ySIdx = boost::fusion::result_of::value_at_c< typename AnsatzVars::Variables,
                                                                   yIdx >::type::spaceIndex;
    static int const uSIdx = boost::fusion::result_of::value_at_c< typename AnsatzVars::Variables,
                                                                   uIdx >::type::spaceIndex;
    static int const pSIdx = boost::fusion::result_of::value_at_c< typename AnsatzVars::Variables,
                                                                   pIdx >::type::spaceIndex;

    class DomainCache : public CacheBase< StepFunctional, DomainCache >
    {
    public:
        DomainCache( StepFunctional const& f_, typename AnsatzVars::VariableSet const& vars_,
                     int flags = 7 )
        ...
        template < int row >
        Scalar
        d1_impl( VariationalArg< Scalar, dim, TestVars::template Components< row >::m > const& arg )
            const
        {
            if ( row == yIdx )
            {
            ...
            if ( row == uIdx )
                return J.template d1< uIdx >( arg ) - p.value * if_( arg.value, u );
            if ( row == pIdx )
                return c.template d1< yIdx >( arg ) - u * if_( arg.value, p.value );
            return 0;
        }

        template < int row, int col >
        Scalar
        d2_impl( VariationalArg< Scalar, dim, TestVars::template Components< row >::m > const& arg1,
                 VariationalArg< Scalar, dim, TestVars::template Components< col >::m > const&
                     arg2 ) const
        {
            if ( row == yIdx && col == yIdx )
            {
                if ( role == RoleOfFunctional::TANGENTIAL )
                    return J.template d2< yIdx, yIdx >( arg1, arg2 ) +
                           c.template d3< yIdx, yIdx, yIdx >( p, arg1, arg2 );
                else
                {
            ...
            if ( row == pIdx && col == uIdx )
                return -if_( arg1.value, p.value ) * if_( arg2.value, u );
            if ( row == uIdx && col == pIdx )
                return -if_( arg1.value, u ) * if_( arg2.value, p.value );

            if ( row == yIdx && col == pIdx )
                return +c.template d2< yIdx, yIdx >( arg2, arg1 );
            if ( row == pIdx && col == yIdx )
                return +c.template d2< yIdx, yIdx >( arg1, arg2 );
            return 0;
        }
    ...
    {
    public:
        BoundaryCache( StepFunctional const& f, typename AnsatzVars::VariableSet const& vars_,
                       int flags = 7 )
        ...
        {
            if ( row == yIdx )
                return gamma * ( p * if_( arg.value, y ) );
            if ( row == pIdx )
                return gamma * ( y * if_( arg.value, p ) );
            return 0;
        }
        ...
        Scalar
        d2_impl( VariationalArg< Scalar, dim, TestVars::template Components< row >::m > const& arg1,
                 VariationalArg< Scalar, dim, AnsatzVars::template Components< col >::m > const&
                     arg2 ) const
        {
            if ( ( row == yIdx && col == pIdx ) || ( row == pIdx && col == yIdx ) )
                return gamma * ( if_( arg1.value, y ) * if_( arg2.value, p ) );
            return 0;
        }
    ...
    explicit StepFunctional( Scalar alpha_, typename AnsatzVars::VariableSet const& ref,
                             Scalar c_ = 1, Scalar d_ = 1, Scalar e_ = 0, Scalar mu_ = 1 )
    ...
    struct D2 : public FunctionalBase< WeakFormulation >::D2< row, col >
    {
        static bool const present =
            !( ( row == yIdx && col == yIdx && role == RoleOfFunctional::PRECONDITIONING ) ||
               ( row == pIdx && col == pIdx ) || ( row == yIdx && col == uIdx ) ||
               ( row == uIdx && col == yIdx ) );
        static bool const symmetric = row == col;
        static bool const lumped =
            ( row == uIdx && col == uIdx && role == RoleOfFunctional::PRECONDITIONING );
    };
...
template < int stateId, int controlId, int adjointId, class RType, class AnsatzVars,
           class TestVars = AnsatzVars, class OriginVars = AnsatzVars >
using NormalStepFunctional = StepFunctional< stateId, controlId, adjointId, RType, AnsatzVars,
                                             TestVars, OriginVars, RoleOfFunctional::NORMAL >;

template < int stateId, int controlId, int adjointId, class RType, class AnsatzVars,
           class TestVars = AnsatzVars, class OriginVars = AnsatzVars >
using TangentialStepFunctional =
    StepFunctional< stateId, controlId, adjointId, RType, AnsatzVars, TestVars, OriginVars,
                    RoleOfFunctional::TANGENTIAL >;

template < int stateId, int controlId, int adjointId, class RType, class AnsatzVars,
           class TestVars = AnsatzVars, class OriginVars = AnsatzVars >
using PreconditioningFunctional =
    StepFunctional< stateId, controlId, adjointId, RType, AnsatzVars, TestVars, OriginVars,
                    RoleOfFunctional::PRECONDITIONING >;
