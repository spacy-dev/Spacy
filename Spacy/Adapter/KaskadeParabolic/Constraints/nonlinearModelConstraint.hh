...
namespace Kaskade
{
    ...
    public:
         ...
         * @param mu linear instability factor  mu zero -> stable as laplace is stable, mu > 0 ->
         * more stable , mu < 0 -> unstable
         */
        explicit NonlinearModelConstraint( Scalar c_ = 0, Scalar d_ = 1, Scalar e_ = 0,
                                           Scalar mu_ = 0 )
            : c( c_ ), d( d_ ), e( e_ ), mu( mu_ )
        {
        }

        void evaluateAt(
            Dune::FieldVector< Scalar, AnsatzVars::template Components< stateId >::m > const& y_,
            Dune::FieldMatrix< Scalar, AnsatzVars::template Components< stateId >::m, dim > const&
                dy_ )
        {
            y = y_;
            dy = dy_;
        ...
        template < int row >
        Scalar
        d1( VariationalArg< Scalar, dim, AnsatzVars::template Components< row >::m > const& arg )
            const
        {
            if ( row != stateId )
                return 0;
        ...
        Scalar
        d2( VariationalArg< Scalar, dim, AnsatzVars::template Components< row >::m > const& arg1,
            VariationalArg< Scalar, dim, AnsatzVars::template Components< col >::m > const& arg2 )
            const
        {
            if ( row != stateId || col != stateId )
                return 0;

            return ( dkappa * arg2.value ) * sp( dy, arg1.derivative ) +
                   kappa * sp( arg2.derivative[ 0 ], arg1.derivative[ 0 ] ) +
                   mu * arg2.value * arg1.value + ddF * arg1.value * arg2.value;
            //      return  d*sp(arg2.derivative,arg1.derivative) + mu*arg2.value*arg1.value;
        }

        template < int id1, int id2, int id3 >
        Scalar
        d3( VariationalArg< Scalar, dim, AnsatzVars::template Components< id1 >::m > const& arg1,
            VariationalArg< Scalar, dim, AnsatzVars::template Components< id2 >::m > const& arg2,
            VariationalArg< Scalar, dim, AnsatzVars::template Components< id3 >::m > const& arg3 )
            const
        {
            if ( id1 != stateId || id2 != stateId || id3 != stateId )
                return 0;
            return ( arg3.value * ( ddkappa * arg2.value ) ) * sp( dy, arg1.derivative ) +
                   ( dkappa * arg2.value ) * sp( arg3.derivative, arg1.derivative ) +
                   ( dkappa * arg3.value ) * sp( arg2.derivative, arg1.derivative ) +
                   dddF * arg1.value * arg2.value * arg3.value;
        }
    ...
    private:
        ...
        Dune::FieldMatrix< Scalar, AnsatzVars::template Components< stateId >::m,
                           AnsatzVars::template Components< stateId >::m >
            ddkappa;
    ...
    public:
        ...
        static constexpr int uSpaceIdx =
        ...
        {
        public:
            DomainCache( NonlinearModelPDE const& h_, typename AnsatzVars::VariableSet const& vars_,
                         int flags = 7 )
                : h( h_ ), data( vars_ )
            {
            }
            ...
            {
                u = boost::fusion::at_c< uIdx >( data.data )
                        .value( boost::fusion::at_c< uSpaceIdx >( evaluators ) );
                du = boost::fusion::at_c< uIdx >( data.data )
                         .derivative( boost::fusion::at_c< uSpaceIdx >( evaluators ) );
                kappa = h.d + h.c * u * u;
                dkappa = h.c * 2 * u;
            ...
            template < int row >
            Scalar d1_impl(
                VariationalArg< Scalar, dim, TestVars::template Components< row >::m > const& arg )
                const
            {
            ...
            Scalar d2_impl(
                ...
                VariationalArg< Scalar, dim, AnsatzVars::template Components< row >::m > const&
                    arg2 ) const
            {
                return ( dkappa * arg2.value ) * sp( du, arg1.derivative ) +
                       kappa * sp( arg2.derivative[ 0 ], arg1.derivative[ 0 ] ) +
                       h.mu * arg2.value * arg1.value + ddF * arg1.value * arg2.value;
            }
        ...
        public:
            ...
            Scalar d0() const
            {
                return penalty * ( u - uDirichletBoundaryValue ) * ( u - uDirichletBoundaryValue ) /
                       2;
            }
         ...
         */
        explicit NonlinearModelPDE( Scalar c_, Scalar d_, Scalar e_, Scalar mu_,
                                    typename DescriptionControl::VariableSet control )
            : c( c_ ), d( d_ ), e( e_ ), mu( mu_ ), control_( control )
        {
        }
        template < int row >
        ...
        int integrationOrder( Cell const& /* cell */, int shapeFunctionOrder, bool boundary ) const
        {
            ...
            {
                int stiffnessMatrixIntegrationOrder =
                    2 * ( shapeFunctionOrder - 1 ) + 2 * shapeFunctionOrder;
                int sourceTermIntegrationOrder =
                    shapeFunctionOrder; // as rhs f is constant, i.e. of order 0
    ...
    public:
        ...
        {
        public:
            BoundaryCache( BoundaryControlledNonlinearModelPDE< RType, AnsatzVars,
                                                                DescriptionControl > const& h_,
                           typename AnsatzVars::VariableSet const& vars_, int flags = 7 )
         ...
         * @param control_ source term of the pde
         */
        explicit BoundaryControlledNonlinearModelPDE(
            Scalar c_, Scalar d_, Scalar e_, Scalar mu_,
            typename DescriptionControl::VariableSet control )
            : c( c_ ), d( d_ ), e( e_ ), mu( mu_ ), control_( control )
        {
        }
