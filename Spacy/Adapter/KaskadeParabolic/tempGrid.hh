...
namespace Spacy
{
    namespace KaskadeParabolic
    {
        ...
        public:
             ...
             */
            TempGrid( Real t_start, Real t_end, unsigned N ) : ts_( t_start ), te_( t_end ), N_( N )
            {
             ...
             * @param t_end upper bound of interval
             * @param N number of timesteps
             * @param lambda scaling factor for distribution of vertices (higher lambda -> more
             * points at beginning)
             */
                ...
                : ts_( t_start ), te_( t_end ), N_( N )
            {
                ...
                Real c =
                    1. / lambda * 1. / ( N - 1 ) * ( exp( lambda * te_ ) - exp( lambda * ts_ ) );
                for ( int i = 0; i < N - 1; i++ )
                {
                    vertexVec_.push_back( 1. / lambda *
                                          log( lambda * c + exp( lambda * vertexVec_[ i ] ) ) );
            ...
            {
                assert( index > 0 );
                vertexVec_.insert( vertexVec_.begin() + index,
                                   ( vertexVec_.at( index - 1 ) + vertexVec_.at( index ) ) / 2. );
            ...
            unsigned getInterval( Real t ) const
            {
                ...
                std::cout << "did not find time " << t << "as vertex is " << vertexVec_.at( i - 1 )
                          << std::endl;
                return 0;
            }
