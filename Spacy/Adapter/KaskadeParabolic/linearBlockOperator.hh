...
namespace Spacy
{
    ...
    namespace KaskadeParabolic
    {
        namespace PDE
        {
           ...
           */
            ...
            class LinearBlockOperator
                : public OperatorBase,
                  public VectorBase,
                  public AddArithmeticOperators< LinearBlockOperator< AnsatzVariableSetDescription,
                                                                      TestVariableSetDescription > >
            {
                ...
                using Range = typename AnsatzVariableSetDescription::
                    template CoefficientVectorRepresentation<>::type;
                using Matrix = ::Kaskade::MatrixAsTriplet< double >;
                using KaskadeOperator =
                    ::Kaskade::MatrixRepresentedOperator< Matrix, Domain, Range >;

                using BlockOperatorCreator =
                    LinearBlockOperatorCreator< AnsatzVariableSetDescription,
                                                TestVariableSetDescription >;

                using VYSetDescription =
                    ::Spacy::KaskadeParabolic::Detail::ExtractDescription_t< AVD, 0 >;
            ...
            public:
               ...
               */
                LinearBlockOperator( std::vector< KaskadeOperator > MassY,
                                     std::vector< KaskadeOperator > A,
                                     const std::vector< Real > dtVec,
                                     const std::vector< std::shared_ptr< Spaces > >& spacesVec,
               ...
               */
                LinearBlockOperator(
                    std::vector< KaskadeOperator > MassY, std::vector< KaskadeOperator > A,
                    const std::vector< Real > dtVec,
                    const std::vector< std::shared_ptr< Spaces > >& spacesVec,
                    const VectorSpace& space,
                ...
                ::Spacy::Vector operator()( const ::Spacy::Vector& x ) const
                {
                    ...
                    for ( auto i = 0u; i < dtVec_.size(); i++ )
                    {
                        res_impl.getCoeffVec_nonconst( i ) =
                            boost::fusion::at_c< 0 >( y_r.at( i ).data );
                    }
                ...
                const auto getKaskOp( std::string type, int timeStep ) const
                {
                    ...
                    if ( !type.compare( "A" ) )
                    {
                        ...
                        std::cout << " NO VALID TYPE GIVEN " << std::endl;
                    return A_.at( 0 );
                }
           ...
           */
                      ...
                      NormOperator< AnsatzVariableSetDescription, TestVariableSetDescription > >
            {
                ...
                using BlockOperatorCreator =
                    NormOperatorCreator< AnsatzVariableSetDescription, TestVariableSetDescription >;
        ...
        {
            /**
           * @brief Linear operator interface for operators associated with time dependent optimal
           * control problems in %Kaskade 7.
           * @tparam AnsatzVariableSetDescription %Kaskade::VariableSetDescription for ansatz
           * variables
           * @tparam TestVariableSetDescription %Kaskade::VariableSetDescription for test variables
           * @see ::Spacy::LinearOperator
           */
                                                                      ...
                                                                      TestVariableSetDescription > >
            {
                ...
                using VYSetDescription =
                    Detail::ExtractDescription_t< AnsatzVariableSetDescription, 0 >;
                using VUSetDescription =
                    Detail::ExtractDescription_t< AnsatzVariableSetDescription, 1 >;
                using VPSetDescription =
                    Detail::ExtractDescription_t< AnsatzVariableSetDescription, 2 >;
            ...
            public:
               ...
               * @param A_t transposed stiffness matrix of constraint for each timestep (to be
               * replaced with applytransposed..)
               * @param B control matrix of constraint for each timestep
               * @param B_t transposed control matrix of constraint for each timestep (to be
               * replaced with applytransposed..)
               * @param dtVec timesteps
               * @param spacesVec Kaskade Space for each timestep
               * @param space OperatorSpace of this linear Operator
               */
                LinearBlockOperator( std::vector< Mytype > My, std::vector< Mutype > Mu,
                                     std::vector< Atype > A, std::vector< Btype > B,
                                     std::vector< ATtype > A_t, std::vector< BTtype > B_t,
                                     std::vector< Masstype > Mass_yy,
                                     std::vector< Masstype > Mass_sd,
                                     const std::vector< Real > dtVec,
                                     const std::vector< std::shared_ptr< Spaces > >& spacesVec,
                                     const VectorSpace& space )
                    : dtVec_( dtVec ), spacesVec_( spacesVec ),
               ...
               */
                LinearBlockOperator(
                    std::vector< Mytype > My, std::vector< Mutype > Mu, std::vector< Atype > A,
                ...
                ::Spacy::Vector operator()( const ::Spacy::Vector& x ) const
                {
                    ...
                    auto x_y =
                        (::Spacy::cast_ref< PSV >( x_ps.component( PRIMAL ) ) ).component( 0 );
                    auto x_u =
                        (::Spacy::cast_ref< PSV >( x_ps.component( PRIMAL ) ) ).component( 1 );
                    ...
                    for ( auto i = 0u; i < dtVec_.size(); i++ )
                    {
                        y_d.emplace_back( CoefficientVectorY(
                            VYSetDescription::template CoefficientVectorRepresentation<>::init(
                                *spacesVec_.at( i ) ) ) );
                        u_d.emplace_back( CoefficientVectorU(
                            VUSetDescription::template CoefficientVectorRepresentation<>::init(
                                *spacesVec_.at( i ) ) ) );
                        p_d.emplace_back( CoefficientVectorP(
                        ...
                        y_r.emplace_back( CoefficientVectorY(
                            VYSetDescription::template CoefficientVectorRepresentation<>::init(
                                *spacesVec_.at( i ) ) ) );
                        y_r.at( i ) *= 0;
                        u_r.emplace_back( CoefficientVectorU(
                            VUSetDescription::template CoefficientVectorRepresentation<>::init(
                                *spacesVec_.at( i ) ) ) );
                        u_r.at( i ) *= 0;
                        p_r.emplace_back( CoefficientVectorP(
                            VPSetDescription::template CoefficientVectorRepresentation<>::init(
                                *spacesVec_.at( i ) ) ) );
                        p_r.at( i ) *= 0;
                    }
                    ...
                    for ( auto i = 0u; i < dtVec_.size(); i++ )
                    {
                        //            copyToCoefficientVector<VYSetDescription>(x_primal_ps.component(i),
                        //            y_d.at(i));
                        //            copyToCoefficientVector<VUSetDescription>(x_primal_ps.component(i
                        //            + dtVec_.size()), u_d.at(i));
                        //            copyToCoefficientVector<VPSetDescription>(x_dual_ps.component(i),
                        //            p_d.at(i));
                        ...
                        //
                        //            std::cout << " In lBO yd before " << y_d.at(i).two_norm() <<
                        //            std::endl;
                        //            std::cout << " In lBO ud before " << u_d.at(i).two_norm() <<
                        //            std::endl;
                        //            std::cout << " In lBO pd before " << p_d.at(i).two_norm() <<
                        //            std::endl;
                        //
                        //            std::cout << " In lBO yr before " << y_r.at(i).two_norm() <<
                        //            std::endl;
                        //            std::cout << " In lBO ur before " << u_r.at(i).two_norm() <<
                        //            std::endl;
                        //            std::cout << " In lBO pr before " << p_r.at(i).two_norm() <<
                        //            std::endl;
                    }
                    // Apply the assembled KaskadeOperators
                    for ( auto i = 0u; i < dtVec_.size(); i++ )
                    {
                        ...
                        if ( i != 0 )
                        {
                            ...
                            //    writeMatlab(A_t_.at(i).get(),"A_t_"+std::to_string(i));mass or
                            //    stiffness)
                        ...
                        if ( i != dtVec_.size() - 1 )
                        {
                            // Mass_sd_.at(i).applyscaleaddTransposed(-1., p_d.at(i + 1),
                            // y_r.at(i));
                    ...
                    for ( auto i = 0u; i < dtVec_.size(); i++ )
                    {
                        //          std::cout<<"Result norm in LBO before
                        //          "<<boost::fusion::at_c<0>(res_y_impl.get(i).data).coefficients().two_norm()<<std::endl;

                        res_y_impl.getCoeffVec_nonconst( i ) =
                            boost::fusion::at_c< 0 >( y_r.at( i ).data );
                        res_u_impl.getCoeffVec_nonconst( i ) =
                            boost::fusion::at_c< 0 >( u_r.at( i ).data );
                        res_p_impl.getCoeffVec_nonconst( i ) =
                            boost::fusion::at_c< 0 >( p_r.at( i ).data );
                        //            std::cout<< "in lbo output "<<
                        //            y_r.at(i).two_norm()<<std::endl;
                        //            std::cout<< "in lbo output "<<
                        //            u_r.at(i).two_norm()<<std::endl;
                        //            std::cout<< "in lbo output "<<
                        //            p_r.at(i).two_norm()<<std::endl;
                        //            std::cout<<"Result norm in LBO
                        //            after"<<boost::fusion::at_c<0>(res_y_impl.get(i).data).coefficients().two_norm()<<std::endl;
                    }
