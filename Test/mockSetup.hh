...
inline auto createMockBanachSpace( bool defaultIndex = false )
{
    ...
    return VectorSpace( Mock::VectorCreator(), Mock::Norm(), "test space", defaultIndex );
}

inline auto createMockHilbertSpace()
{
    return Spacy::makeHilbertSpace( Mock::VectorCreator(), Mock::ScalarProduct(), "test space" );
}

inline auto firstGlobalId()
{
    return 1u;
}
inline auto secondGlobalId()
{
    return 0u;
}
inline auto thirdGlobalId()
{
    return 5u;
}
inline auto notGlobalId()
{
    return 42u;
}
...
inline auto makeProductHilbertSpaceWithMap()
{
    ...
    for ( auto i = 0u; i < numberOfVariables(); ++i )
        spaces.push_back( std::make_shared< VectorSpace >( createMockHilbertSpace() ) );
    return makeTuple( ProductSpace::makeHilbertSpace( spaces, createGlobalIds() ), spaces,
