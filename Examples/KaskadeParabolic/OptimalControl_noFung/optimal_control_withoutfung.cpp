...
int main(int argc, char *argv[])
{
  ...
  unsigned int initialRefinements = getParameter(pt, "initialRefinements", 4);
  ...
  using std::cout;
  using std::endl;
  cout << "cPara: "<<c<<" dPara: " << d << " ePara: " << e << " muPara: " << mu << " alpha:" << alpha << endl;
...
//    using TangentialFunctionalDefinition =  TangentialStepFunctional<stateId,controlId,adjointId,double,Descriptions>;
  ...
  obj << "alpha: " <<alpha << ", c: " <<c<<", d: " <<d<<", mu: "<<mu<<", t_end: "<<t_end<<", exactgrid: "<<exactgrid<<", uni/expgrid: "<<exp_uni_grid;



  //####################### Exact SOLUTION #######################

  ::Spacy::KaskadeParabolic::GridManager<Spaces> gm1 (exactgrid,::Spacy::Real{t_end},initialRefinements,FEorder);
  ...
  auto t_func1 = Spacy::KaskadeParabolic::makeC2Functional(tangentialFuncGenerator,gm1,domain1);

  typename Descriptions::VariableSet x01(Descriptions(*gm1.getSpacesVec().at(0),{"y","u","p"}));
  ...
  ::Spacy::KaskadeParabolic::GridManager<Spaces> gm2 (exp_uni_grid,::Spacy::Real{t_end},initialRefinements,FEorder);
  ...
  auto t_func2 = Spacy::KaskadeParabolic::makeC2Functional(tangentialFuncGenerator,gm2,domain2);

  typename Descriptions::VariableSet x02(Descriptions(*gm2.getSpacesVec().at(0),{"y","u","p"}));
  interpolateGloballyWeak<PlainAverage>(boost::fusion::at_c<0>(x02.data),
                                        makeWeakFunctionView( [](auto const& cell, auto const& xLocal) -> Dune::FieldVector<double,1>
  {
    auto x = cell.geometry().global(xLocal);
  ...
  ::Spacy::KaskadeParabolic::GridManager<Spaces> gm3 (exp_uni_grid,::Spacy::Real{t_end},initialRefinements,FEorder, "exponential",expgridparam);
  ...
  auto t_func3 = Spacy::KaskadeParabolic::makeC2Functional(tangentialFuncGenerator,gm3,domain3);

  typename Descriptions::VariableSet x03(Descriptions(*gm3.getSpacesVec().at(0),{"y","u","p"}));
  interpolateGloballyWeak<PlainAverage>(boost::fusion::at_c<0>(x03.data),
                                        makeWeakFunctionView( [](auto const& cell, auto const& xLocal) -> Dune::FieldVector<double,1>
  {
    auto x = cell.geometry().global(xLocal);
  ...
  Spacy::KaskadeParabolic::OCP::printNormSolution(uniform_solution,n_func2.hessian(uniform_solution),gm2, "uni");
  Spacy::KaskadeParabolic::OCP::printNormSolution(exp_solution,n_func3.hessian(exp_solution),gm3, "exp");

  std::cout<< "distance to uni sol "<<std::endl;
  Spacy::KaskadeParabolic::OCP::printErrorDistribution(exact_solution, uniform_solution, NormFunc, gm1,  "uni");
  std::cout<< "distance to exp sol "<<std::endl;
  Spacy::KaskadeParabolic::OCP::printErrorDistribution(exact_solution, exp_solution, NormFunc, gm1,  "exp");
