...
//  using L2Space = FEFunctionSpace<ContinuousLagrangeMapper<double,Grid::LeafGridView> >;
  ...
  ::Spacy::KaskadeParabolic::GridManager<Spaces> gm (11,::Spacy::Real{3.},initialRefinements,FEorder);
...
//  using TangentialFunctionalDefinition =  TangentialStepFunctional<stateId,controlId,adjointId,double,Descriptions>;
  ...
  typename Descriptions::VariableSet x0(Descriptions(*gm.getSpacesVec().at(0),{"y","u","p"}));
  interpolateGloballyWeak<PlainAverage>(boost::fusion::at_c<0>(x0.data),
  ...
  //if distributed control
//  using HeatFunctionalDefinition = NonlinearModelPDE<double,::Spacy::KaskadeParabolic::Detail::ExtractDescription_t<Descriptions,0> , ::Spacy::KaskadeParabolic::Detail::ExtractDescription_t<Descriptions,1>  >;
  ...
  ::Spacy::cast_ref<::Spacy::KaskadeParabolic::C1Operator<HeatFunctionalDefinition> >(A).setInitialCondition(boost::fusion::at_c<0>(x0.data).coefficients());
