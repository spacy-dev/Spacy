...
int main(int argc, char *argv[])
{
    ...
    auto mg = makeBPX(fL.assembler().get<2,0>(), gridManager);
    auto ABPX = makeSingleBlockPreconditioner<SubY,SubP>(adjointSpace,stateSpace,std::make_unique<decltype(mg)>(std::move(mg)));
...
//    writeVTKFile(gridManager.grid().leafGridView(), xout, outfilename, options, order);
    ...
    if(!(parameters.useMINRES || parameters.usePDP))
    {
        std::cout << "Solving some forward problem..." << std::endl;
        ...
        std::cout << "Eigs: A:[" << eigMinA << "," << eigMaxA << "]" << "Iterations: " << ACg.getIterations() << std::endl;        
        std::cout << boost::timer::format(CGTimer.elapsed()) << std::endl;
    }
    
    if(!parameters.useMINRES) return 0; 
    ...
    Spacy::Vector x(zero(totalSpace));  
        ...
        auto ACheb = Spacy::Chebyshev::ChebyshevPreconditioner(A,ABPX,eigMaxA,eigMinA);
        ACheb.setRelativeAccuracy(chebyshevAcc);
        std::cout << "Eigs: :A:[" << eigMinA << "," << eigMaxA << "]" << std::endl;
    ...
    if(!useDirectBlockPreconditioner && useSchurPreconditioner)   
    {
        ...
        auto mgM = makeBPX( NumaBCRSMatrix<Dune::FieldMatrix<double, dim, dim>>(*bcrsM,false), gridManager);
        
            auto My = makeSymmetricCallableBlock<Functional>(fL,stateSpace);
//           auto mgM = makeBPX(NumaBCRSMatrix<Dune::FieldMatrix<double, dim, dim>>(fL.assembler().get<1,1>(),false), gridManager); 
...
//        auto mgM = makeBPX( NumaBCRSMatrix<Dune::FieldMatrix<double, dim, dim>>(*bcrsM,false), gridManager);
        ...
        for(int i=0; i<4; ++i)
        {
            ...
            std::cout << "Eigs: :M:[" << eigMinM << "," << eigMaxM << "]" << std::endl;
            ...
            std::cout << "Number of BPX Applications for My: per MINRES iteration: " << MCheb.getIterations() << " total: " << MCheb.getIterations() * solverMINRES.getIterations() << std::endl;
    ...
    std::cout << boost::timer::format(minresTimer.elapsed()) << std::endl;
