...
namespace Kaskade
{
    ...
    class OperatorWithConstantSource : public CacheBase< Operator, OperatorWithConstantSource< Operator, Integrand, state_index > >
    {
    ...
    public:
        OperatorWithConstantSource( const Operator& F, typename AnsatzVars::VariableSet const& x, int ) : f_( F.f_ ), x_( x ), g_( 1 )
        {
        }
        ...
        void evaluateAt( const Position&, Evaluators const& evaluators )
        {
            ...
            f_.template update< state_index >(
                std::make_tuple( at_c< state_index >( x_.data ).value( at_c< state_space_index >( evaluators ) ),
    ...
    class FunctionalWithConstantSource : public CacheBase< Functional, FunctionalWithConstantSource< Functional, Integrand, state_index > >
    {
    ...
    public:
        FunctionalWithConstantSource( const Functional& F, typename AnsatzVars::VariableSet const& x, int )
